	...
	public static void attemptStall(final Player p, final int objectId, final int x, final int y) {
		...
		for (final stallData s : stallData.values()) {
			if(objectId == s.getObject()) {
				...
				if (p.playerLevel[SkillData.THIEVING.getId()] < s.getLevel()) {
				...
				if(Misc.random(4) == 1 && p.playerLevel[SkillData.THIEVING.getId()] < 99) {
					failGuards(p);
					return;
				}
				...
				p.getPlayerAssistant().addSkillXP((int) s.getXp(), SkillData.THIEVING.getId());
